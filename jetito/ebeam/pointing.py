...
class pointing_analysis:
    ...
    def __init__(self, filename, rescale=False, image_calib=18.3e-3, d_target_screen = 1.45):        
        ...
        Args:
            ...
            image_calib (double, optional): Calibration of the near-field image in units of length/pixel (typical: mm/pixels). Defaults to 18.5e-3.
            d_target_screen (double, optional): Distance from the target to the screen where the pointing image was recorded (typical: meters). Defaults to 1.45.
        """
    ...
    def crop_image(self, left=100, right=1260, top=795, bottom=1970, save_file=None, **kwargs):
        ...
        Args:
            left (int, optional): Pixel position for starting cropping the image from the left. Defaults to 580.
            right (int, optional): Pixel position for starting cropping the image from the right. Defaults to 830.
            top (int, optional): Pixel position for starting cropping the image from the top. Defaults to 600.
            bottom (int, optional): Pixel position for starting cropping the image from the bottom. Defaults to 740.
            save_file (string, optional): Path of the file to save the cropped focus. Please provide this path. Defaults to None.
        """
        ...
        if verbose:
            print("Shape of the cropped image: %d x %d"%(self.image_crop.shape[0], self.image_crop.shape[1]))
    ...
    def calculate_pointing_parameters(self, init_guess=(500, 2.214, 3.7, 385, 175), **kwargs):
        ...
        x0, y0, amplitude, sigma_x, sigma_y, rotation angle, fwhm_x, fwhm_y are calculated and stored in variables.
        If verbose = True, the fit parameters are also printed in the console.
    

        Args:
            init_guess (tuple, optional): Initial guess parameter for the fit function. (amplitude, sigma_x, sigma_y, theta, offset). Defaults to (100, 10, 10, 0, 0).
        ...
        try:
            ...
            if verbose:
                print("Maximum at: x = %.3f um and y = %.3f um"%(self.XX[idx_max_horizontal, idx_max_vertical],
                                                                 self.YY[idx_max_horizontal, idx_max_vertical]))
            ...
            self.popt_fit, self.pcov_fit = gf.fit2d(XX = self.XX,
                                                    ...
                                                    initial_guess = (init_guess[0],
                                                                     self.XX[idx_max_horizontal, idx_max_vertical],
                                                                     self.YY[idx_max_horizontal, idx_max_vertical],
                                                                     init_guess[1],
                                                                     init_guess[2],
                                                                     init_guess[3],
    ...
    def plot_fields_fit(self, save_file=None, xlim=None, ylim=None, clim=None, **kwargs):
        ...
        Args:
            save_file (string, optional): Path and file name, if image is supposed to be saved. Defaults to None.
            xlim (tuple, optional): Defines the limits of the X-axis of the 2D-plot. Defaults to None.
            ylim (tuple, optional): Defines the limits of the Y-axis of the 2D-plot. Defaults to None.
            clim (tuple, optional): Defines the limits of the colorbar of the 2D-plot. Defaults to None.
        """
        ...
        _=ax.text(0.02, 0.125, text_legend, horizontalalignment='left', color='white', fontsize=8, weight='bold',
        ...
        _=ax.text(0.6, 0.125, text_legend2, horizontalalignment='left', color='white', fontsize=8, weight='bold',
                verticalalignment='center', transform=ax.transAxes)
        ...
        try:
            ...
            _=ax.text(0.75, 0.95, text_legend3, horizontalalignment='left', color='white', fontsize=8, weight='bold',
                    verticalalignment='center', transform=ax.transAxes)
        except:
    ...
    def calculate_charge(self,
        ...
        Args:
            screen_yield (double, optional): Calibrated screen yield. Typically in photons/sr/pC. Defaults to 8.25e9 (Lanex Biomax MS).
            camera_calib (double, optional): Calibration of the camera photon/counts conversion. Typically in photons/counts. Defaults to 6.5.
            transmission_loss (double, optional): Transmission loss of the optical imaging system in the experiment. Accounts for the mirror and objectiv losses for example. Defaults to 5e-3.
            lens_focal_length (double, optional): Focal lens (in mm) of the objective CCTV/Macro lens used. Typically in mm. Defaults to 25.
            lens_fnumber (double, optional): F/number of the objective CCTV/Macro lens used in the experiment. Defaults to 5.6.
            dist_cam_screen (double, optional): Distance (in meters) of the objective lens and the screen in the experiment. Used for calculating the imaging solid angle. Typically in meters. Defaults to 15e-2.
