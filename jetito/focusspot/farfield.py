...
class farfield_calculator:
    """Class to calculate the far-field (2D Fourier Transform) of a near-field image.
       ...
       Please see the scrip test_farfield.py in the tests folder to have an example of how to work with this class.
    """
    ...
    def __init__(self, filename, image_calib=0.147e-3):
        ...
        Args:
            ...
            image_calib (double, optional): Calibration of the near-field image in units of length/pixel (typical: um/pixels). Defaults to 0.147e-3.
        """
    ...
    def add_points(self, dim = 2**13, show_img = False, save_file="../tests/results/farfield/"):
        ...
        Args:
            ...
            save_file (str, optional): Path and file name where to save the plot. Defaults to "../tests/results/farfield/".
        ...
        self.near_field = cv2.copyMakeBorder(self.img_nearfield, top=top_bottom_px, bottom=top_bottom_px,
                                                      left=left_right_px, right=left_right_px, 
    ...
    def calculate_far_field(self, wavelength=800e-9, 
                            distance=2.5,
                            norm=True):
        """Calculate the far field from the near field of a light beam using 2D FFT with interpolation.

        Args:
            wavelength (float, optional): Wavelength of the light beam in meters. Defaults to 800e-9.
            distance (float, optional): Distance from the near field to the far field in meters. Defaults to 2.5.
        ...
        try:
            ...
            delta_fx = 1/(self.near_field.shape[0] * (self.image_calib)) * 2 * np.pi; print("delta_fx = %.3e "%delta_fx)
            delta_fy = 1/(self.near_field.shape[1] * (self.image_calib)) * 2 * np.pi; print("delta_fy = %.3e "%delta_fy)
    ...
    def plot_fields_fit(self, save_file):
        ...
        _=ax.text(0.1, 0.12, text_legend, horizontalalignment='left', color='white', fontsize=10, weight='bold',
        ...
        text_legend2 = ("$\sigma_y$ = %.1f \u03BCm\n" + 
                        ...
                        "q-factor = %.1f %%")%( self.popt_fit[4], 
                                                ...
                                                self.q_factor)
        _=ax.text(0.625, 0.12, text_legend2, horizontalalignment='left', color='white', fontsize=10, weight='bold',
                verticalalignment='center', transform=ax.transAxes)
